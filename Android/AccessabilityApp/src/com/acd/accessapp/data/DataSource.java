...
package com.acd.accessapp.data;
...
import com.acd.accessapp.R;
import com.acd.accessapp.Settings;
...
//		new AppData(DataSource.this);
//		settings = AppData.getSettings();
		
//		Toast.makeText(this, ""+MixView.sets.Toilets, 1000).show();
		...
		if (!ret.startsWith("file://")) {
			...
			case WIKIPEDIA:
				/*float geoNamesRadius = radius > 20 ? 20 : radius; */// Free
																	// service
//				ret += "?latitude=" + lat + "&longitude=" + lon ;
				
				@SuppressWarnings("rawtypes")
				HashSet type = new HashSet();
				if(MixView.sets.Buildings)
				 {
					type.add("2");
				 }
				if(MixView.sets.Toilets)
				{
					type.add("1");
				}
				if(MixView.sets.Super_Stops)
				{
					type.add("3");
				}
				String temp = type.toString().trim().replaceAll(" ", "");
				ret += "?latitude=" + lat + "&longitude=" + lon +"&type="+ temp.substring(1,temp.toString().length()-1);// limited
